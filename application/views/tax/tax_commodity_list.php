...
echo "<tr><td>";echo form_submit('delete_ad',$this->lang->line('delete'));echo "</td><td>";echo form_submit('add_tax',$this->lang->line('add'));echo "</td><td>";echo form_submit('cancel',$this->lang->line('cancel')); echo "</td></tr>";
    ...
    foreach ($row as $trow ){ ?>
<tr><td><input type="checkbox" name="mycheck[]" value="<?php echo $trow->id ?>" /></td><td><?php echo $trow->code  ?></td><td><?php echo $trow->schedule ?></td><td><?php echo $trow->tax_area  ?></td><td><?php foreach ($tax as $stax) {if($trow->tax==$stax->id){ echo $stax->value."% -".$stax->type; }  } ?></td>
