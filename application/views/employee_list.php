...
if($count!=0){
      if($_SESSION['admin']==2){?><table >
        ...
        </td> <td style="width: 150;margin-left: 150px"><?php if($b_row->user_active==0){ ?><a href="<?php echo base_url() ?>index.php/employees/to_deactivate_user/<?php echo $erow->id ?>">Deactivate</a> <?php } else{ ?><a href="<?php echo base_url() ?>index.php/employees/to_activate_user/<?php echo $erow->id ?>"> Activate</a> <?php } ?></td>
        ...
        <td><a href=" <?php echo base_url() ?>index.php/employees/delete_employee_details_in_admin/<?php echo $erow->id ?>"><?php echo $this->lang->line('delete') ?></a></td>
    </tr><?php }}}?></table>
<tb><?php echo form_submit('activate',$this->lang->line('activate'))?></td><tb><?php echo form_submit('deactivate',$this->lang->line('deactivate'))?></td><td><input type="submit" name="delete_user_for_admin" value="<?php echo $this->lang->line('delete') ?>"></td><tb><input type="submit" name="Add_employee" value="<?php echo $this->lang->line('add_new_user') ?>"></td><td><?php echo form_submit('BacktoHome',$this->lang->line('back_to_home')) ?></td>
...
<?php }
    ...
    <tb><input type="submit" name="Add_employee" value="<?php echo $this->lang->line('add_new_user') ?>"></td><td><?php echo form_submit('BacktoHome',$this->lang->line('back_to_home')) ?></td>
