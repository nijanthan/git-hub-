<?php  if($_SESSION['Item_per']['add']==1){
     ...
     echo "<tr><td>"; echo form_label($this->lang->line('cate_name'));echo "</td><td>"; if(count($srow)>0 ){ ?><select><?php foreach ($srow as $cs_row){        foreach ($sb_row as $csk_row){ if($cs_row->supplier_id == $csk_row->id){ ?><option name="<?php echo $csk_row->id  ?> "> <?php echo $csk_row->company_name  ?> </option><?php }}}echo "</select>";?> <a href="<?php echo base_url() ?>index.php/items/add_supplier">Add supplier</a> <?php }else{?> <a href="<?php echo base_url() ?>index.php/items/add_supplier">Add Supplier</a> <?php }  echo "</td></tr>";
