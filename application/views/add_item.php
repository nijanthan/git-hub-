...
$( "#date_end_picker" ).datepicker();
     ...
     echo "<tr><td>"; echo form_label($this->lang->line('cate_name'));echo "</td><td>"; if(count($row)>0 ){ ?><select  style="width:150" name="category"><?php foreach ($row as $c_row){        foreach ($crow as $irow){ if($c_row->category_id == $irow->id){ ?><option name="<?php echo $irow->id  ?> " value="<?php echo $irow->id  ?>"> <?php echo $irow->category_name  ?> </option><?php }}}echo "</select>";?> <?php }  echo "</td></tr>";
