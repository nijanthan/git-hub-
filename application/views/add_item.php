<?php  if($_SESSION['Item_per']['add']==1){
     ...
     echo "<tr><td>"; echo form_label($this->lang->line('start_date'));echo "</td><td>";echo form_input('start_date',set_value('start_date'),'id="start_date" autofocus');echo "</td></tr>";
     echo "<tr><td>"; echo form_label($this->lang->line('end_date'));echo "</td><td>";echo form_input('end_date',set_value('end_date'),'id="end_date" autofocus');echo "</td></tr>";
     ...
     echo "<tr><td>"; echo form_label($this->lang->line('cate_name'));echo "</td><td>"; if(count($row)>0 ){ ?><select><?php foreach ($row as $c_row){        foreach ($crow as $irow){ if($c_row->category_id == $irow->id){ ?><option name="<?php echo $irow->id  ?> "> <?php echo $irow->category_name  ?> </option><?php }}}echo "</select>";?> <a href="<?php echo base_url() ?>index.php/items/add_new_category">Add Category</a> <?php }else{?> <a href="<?php echo base_url() ?>index.php/items/add_new_category">Add Category</a> <?php }  echo "</td></tr>";
     echo "<tr><td>"; echo form_label($this->lang->line('cate_name'));echo "</td><td>"; if(count($srow)>0 ){ ?><select><?php foreach ($srow as $cs_row){        foreach ($sb_row as $csk_row){ if($cs_row->supplier_id == $csk_row->id){ ?><option name="<?php echo $csk_row->id  ?> "> <?php echo $csk_row->company_name  ?> </option><?php }}}echo "</select>";?> <a href="<?php echo base_url() ?>index.php/items/add_new_supplier">Add supplier</a> <?php }else{?> <a href="<?php echo base_url() ?>index.php/items/add_new_supplier">Add Supplier</a> <?php }  echo "</td></tr>";
